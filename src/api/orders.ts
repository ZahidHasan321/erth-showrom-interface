...
export const deleteOrder = async (orderId: string): Promise<ApiResponse<void>> => {
  ...
  const response = await fetch(`${BASE_URL}/airtable/${TABLE_NAME}/records/${orderId}`, {
    method: 'DELETE',
  });
...
export const getPendingOrdersByCustomer = async (
  ...
  if (response.data && Array.isArray(response.data)) {
    ...
    const sortedData = pendingOrders.sort((a, b) => {
      const dateA = a.fields.OrderDate ? new Date(a.fields.OrderDate).getTime() : 0;
      const dateB = b.fields.OrderDate ? new Date(b.fields.OrderDate).getTime() : 0;
 ...
 */
export const getCompleteOrderDetails = async (orderIdValue: string): Promise<ApiResponse<any>> => {
  const BASE_URL = import.meta.env.VITE_API_BASE_URL;
  const response = await fetch(`${BASE_URL}/airtable/order_details/${orderIdValue}`);

  if (!response.ok) {
    throw new Error(`Failed to fetch complete order details for ${orderIdValue}`);
  }
