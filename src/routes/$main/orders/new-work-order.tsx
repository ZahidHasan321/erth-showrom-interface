...
function NewWorkOrder() {
	...
	const handleFabricSelectionSubmit = (data: {
		...
		if (stitchingPrice) OrderForm.setValue("charges.stitching", stitchingPrice * data.fabricSelections.length)
	};
	...
	const totalShelveAmount = products?.reduce((acc, p) => acc + (p.quantity ?? 0) * (p.unitPrice ?? 0), 0) ?? 0;
						...
						{index === 5 && (
							<PaymentTypeForm
								...
								onSubmit={() => {
									...
									const isAddressDefined = Object.values(demographicsForm.getValues().address).every(value => value !== undefined && value.trim() != "")
